...
async function run() {
  try {
    ...
    // Fetch a specific thesis by title
    app.get('/all_thesis/:title', async (req, res) => {
      const title = req.params.title; // Get the title from the URL parameter
      const thesis = await thesisCollection.findOne({ title: title }); // Query thesisCollection for the matching title
      if (thesis) {
        res.send(thesis); // Send the found thesis back to the client
      } else {
        res.status(404).send({ error: 'Thesis not found' }); // Handle case where no thesis is found
      }
    });
    ...
    app.patch('/student_details/:student_id/assign-supervisor', async (req, res) => {
      ...
      try {
          // Fetch thesis details for the student
          const thesisData = await thesisCollection.findOne({ student_ids: studentId });
          if (!thesisData) {
              return res.status(404).send({ message: 'Thesis details not found for the student.' });
          }
  
          // Get all student IDs from the thesis except the current student
          const allStudentIds = thesisData.student_ids.filter(id => id !== studentId);
  
          // Update supervisor for the current student
          const updateCurrentStudent = await studentCollection.updateOne(
              { student_id: studentId },
              { $set: { assignedSupervisor } }
          );
  
          if (updateCurrentStudent.matchedCount === 0) {
              return res.status(404).send({ message: 'Student not found.' });
          }
  
          // Update supervisor for teammates
          const updateTeammates = await studentCollection.updateMany(
              { student_id: { $in: allStudentIds } },
              { $set: { assignedSupervisor } }
          );
  
          res.send({
              message: 'Supervisor assigned successfully to the student and teammates.',
              updatedCount: updateTeammates.modifiedCount + 1, // Includes the current student
          });
      } catch (error) {
